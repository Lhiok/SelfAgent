...
from session.changes import extract_change_from_call, merge_changes_by_path
from session.detail import DETAIL_OFF, format_result_detail, format_step_detail
from session.types import AgentResult, AgentStep
...
class ProgressMixin:
    ...
    def _emit_progress(self: Agent, event: dict[str, Any]) -> None:
    ...
    def _step_progress_payload(self: Agent, step: AgentStep) -> dict[str, Any]:
    ...
    def _emit_step(self: Agent, step: AgentStep) -> None:
    ...
    def _finalize_result(self: Agent, result: AgentResult) -> AgentResult:
