...
class MainWindow(QMainWindow):
    def __init__(self, *, config_path: Path | None = None) -> None:
        ...
        mode_cap = QLabel("模式")
        mode_cap.setObjectName("PanelCaption")
        self.mode_combo = QComboBox()
        self.mode_combo.addItems(["agent", "plan"])
        self.mode_combo.currentTextChanged.connect(self._on_mode_changed)
        detail_cap = QLabel("细节")
        detail_cap.setObjectName("PanelCaption")
        ...
        top.addWidget(mode_cap)
        top.addWidget(self.mode_combo)
        top.addWidget(detail_cap)
        ...
        if not self.store.list_workspaces():
            self.chat.set_status("请先添加工作目录（侧栏 +）")
